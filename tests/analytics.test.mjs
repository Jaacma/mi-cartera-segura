...
test('calcula rentabilidad y riesgo con una serie verificada',()=>{
  const start=Date.UTC(2025,0,1),history=Array.from({length:260},(_,index)=>({date:new Date(start+index*86_400_000).toISOString().slice(0,10),nav:100*Math.pow(1.0005,index)*(1+0.01*Math.sin(index/5))}));
  const result=buildAnalytics({isin:'TEST',name:'Fondo',benchmark:'Índice'},history);
  assert.equal(result.observations,260);
